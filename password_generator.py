...
def main():
    length = int(
        input("Milyen hosszú legyen a jelszó összesen (hány karakter)? "))
    include_numbers = input(
        "Legyenek-e benne számok (igen/nem)? ").lower() == "igen"
    include_special_chars = input(
        "Legyenek-e benne speciális karakterek (igen/nem)? ").lower() == "igen"
